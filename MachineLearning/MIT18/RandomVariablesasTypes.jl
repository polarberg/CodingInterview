...
bell_curve(x) = exp(-x^2 / 2) / √(2π) # formula for standard normal

# ╔═╡ c76cd760-8fd5-11eb-3500-5d15515c33f5
bell_curve(x, μ, σ) = bell_curve( (x - μ) / σ ) / σ # evaluating original. Divide by sigma to equal 1 
...
# ╔═╡ 4bd1f323-0f86-4b44-8942-b5e46c99ec19
1/ √(2π*σ^2) # top of bell curve 
...
mean(G1 + G2) == mean(G1) + mean(G2) # mean of the sum is the sum of the means
...
Statistics.mean(S::SumOfTwoRandomVariables) = mean(S.X1) + mean(S.X2) # mean for Bernoulli or Gaussian previously defined 
...
# ╔═╡ 83ef61b2-717d-4a52-bc71-7695cb380913
Statistics.var(total)
...
# can see abstract syntax tree in action
...
S = sum(Bernoulli(0.25) for i in 1:3) # sum of iterator 
# more memory efficient - doesn't store entire vector

# ╔═╡ f3a2afae-56c8-4323-9f53-c8bb110529f4
sum( [Bernoulli(0.25), Bernoulli(0.25), Bernoulli(0.25)] ) # sum of vector 
