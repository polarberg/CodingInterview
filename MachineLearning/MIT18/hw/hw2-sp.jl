...
student = (name = "Austin Nguyen", kerberos_id = "SOLUTIONS")
...
# ╔═╡ 66bec20e-8126-11eb-0e2f-257536db50ff
...
# ╔═╡ bf170c80-881d-11eb-3567-ffe4dff40b87
typeof(v)
...
function convolve(v::AbstractVector, k)
	...
	for i in 1:length(v) # for each i, a[i] = ⅀v[j]*k 
		...
		z = 1
		for j in -l:l
			#tmpSum += extend(v, i-j) * k[z] # k[i,m] * [m,n] = [i,n]
...
# ╔═╡ bb321c10-881b-11eb-3be0-f9d31fb020ae
p=1

# ╔═╡ c12f5012-881b-11eb-2184-d5fb950aa2bc
N = 2(p) + 1

# ╔═╡ c133bce0-881b-11eb-207d-d9c241ed5816
[(j,i)  for i in -p:p, j in -p:p]

# ╔═╡ c13e1d20-881b-11eb-1650-f1e8a93862c3
...
colored(convolve(test_image_with_border, K_test))
...
# ╔═╡ 69cf5250-8ceb-11eb-0e1d-ddce22da958a
function gaussian_kernel_2D(n; σ = 3)
	ans = [gauss(i,j,σ=σ) for i in -n:n, j in -n:n]
	
	return ans ./ sum(ans) # normalize each n 
end

# ╔═╡ da2125a0-924b-11eb-21e4-cb212263c448
gaussian_kernel_2D(3, σ = 0.84089642)

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image; σ=3, l=5)
	k= gaussian_kernel_2D(l, σ=σ) # create k matrix from l 
...
function with_sobel_edge_detect(image)
	kernal_x = horizontal_edge_detect_kernal()
	kernal_y = vertical_edge_detect_kernal()
	
	x_edges = convolve(image, kernel_x) 
	y_edges = convolve(image, kernel_y)
