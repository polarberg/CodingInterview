...
# ╔═╡ 4ecb194b-271c-44b5-ab3c-01ead4132c2f
function formula(j) 
    return (j*(1/j))!=1
end

# ╔═╡ e257c408-7ecf-4a5c-bb83-0dd9955c6800
filter(formula, 1:1000)
...
# ╔═╡ 4e342e38-f193-44dd-89a2-b71320c727eb
FirstRankOneMatrix([1,2,3])

# ╔═╡ 3f1764a0-579d-43ea-a4b6-4b06635ccda1
[i*y for i in [1,2,3], y in [1,2,3]]
...
# ╔═╡ 8de46fd9-fad2-4083-9e1b-728931e616f6
...
function matvec(M::LowRankMatrix, x)
	...
	return answer 
	# return [sum([ M[1,i] * x[i] for i in 1:size(M,1)]) * M[j,1] for j in 1:size(M,1)] 
end
...
# ╔═╡ 844a5b18-b81f-4470-b373-492003f8423b
svd(A).S

# ╔═╡ ed4b7ea5-3266-466d-a817-7ab2cc82ac9c
count(i->(i>1),LinearAlgebra.svd(A).S)
...
function k_rank_ones(k, m, n)
	A = sum(RankOneMatrix(rand(m),rand(n)) for i in 1:k) # returns RankOneMatrix{Float64}
	return A
end
...
# ╔═╡ b56e371c-dbb5-48f9-b9c4-9d0805ca882e
numerical_rank(k_rank_ones(1,3,3)) #

# ╔═╡ 9174bb6f-39a9-42ab-9481-b327d6cc1fc6
