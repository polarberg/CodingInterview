...
# ╔═╡ bef72215-d2d6-4392-a9e7-c49250e4940e
using Statistics
...
student = (name = "Austin Nguyen", kerberos_id = "jazz")
...
# ╔═╡ 68daf7f8-e3ff-4f42-9dae-1d4568135963
ks = collect(keys(test_counts))

# ╔═╡ 860eb0de-45eb-4063-ac40-c6423ca12255
...
# ╔═╡ 9945f3f4-c444-4ea5-89c3-e053a50dd9b2
bernoulli(.25)
...
# ╔═╡ 91366e33-a1c9-4823-a2d0-89ed3a75d8b3
...
# ╔═╡ 668d8035-f583-4040-9dfb-6a2b7ad4f17e
geometric(0.25)
...
# ╔═╡ 4eb2539e-16c7-4b52-8107-fa749e043db0
...
@bind numElem Slider( 0 : 1000 : 100000 )

# ╔═╡ fffa5027-967e-463c-aaf1-a33726fefc8b
numElem
...
# ╔═╡ 656e1bfe-d372-4455-a8f0-659a6a15c432
@bind prob_P_vs_tP Slider( 0.01 : 0.01 : 0.3 )

# ╔═╡ 2ea23a62-710e-4fba-8994-375a2f3271ac
...
md"""
```math
sum = \frac{a_1}{(1-r)} , a_1 = p, a_2 = p(1-p)
```
```math
...
# ╔═╡ 5966be29-57eb-4502-991d-c79865cf3c3c
PlotExp(prob,numElem)

# ╔═╡ 8ea47b6d-4842-455c-abbb-f614a698f66e
plot!(1:50, Ps)
...
# ╔═╡ 58cab9fa-7ab8-4cf3-aeda-72fe210b5f55
Ps

# ╔═╡ 25219603-85e9-4e21-9d94-4caed66c2b66
cumSum1 = cumulative_sum(Ps)

# ╔═╡ de96ee9f-2dac-4ae1-8d93-8c1927b94aa0
...
# ╔═╡ a3ae1e0f-6344-4d6f-8983-5d69049681f2
function cum_sumAnal2(p::Real,n::Int)
	return 1-(1-p)^n
end 

# ╔═╡ 22fd9e2d-7974-4332-b9ba-9f6ee60e38d6
cumSum2 = [cum_sumAnal(0.25,i) for i in 1:50]

# ╔═╡ 1cc2ac6b-66fa-4112-b47a-6463266089cd
cumSum3 = [cum_sumAnal2(0.25,i) for i in 1:50]

# ╔═╡ 8bd9930a-5806-4e11-95fc-0c5c598178c0
cumSum1 - cumSum2
...
# ╔═╡ 5a45a881-a043-4052-bd40-8de8ef88ffa1
md"""
Defining $C_{0} = 0$ for mathematical convenience, the chosen bucket $n$ is defined by:

$$\begin{alignat*}{1}
C_{n-1} &\le \quad  r &< C_{n} \\
1 - (1 - p)^{n-1} &\le \quad r & <   1 - (1 - p)^{n} \\
\ln((1 - p)^{n-1}) &\ge \ln(1 - r) &>  \ln((1 - p)^{n})\\
n-1 &\le \frac{\ln(1-r)}{\ln(1-p)}  &<  n
\end{alignat*}$$

So choose $n$ such that it is larger than the log quotient and $n - 1$ is smaller equal the log quotient. Therefore
...
# ╔═╡ 300f5a06-5111-4e33-a469-467911177190
probs = 10^-10 
