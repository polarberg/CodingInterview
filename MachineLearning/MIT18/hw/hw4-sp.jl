...
student = (name = "Austin Nguyen", kerberos_id = "Austin")
...
# ╔═╡ fbf0b5a7-fc1e-42a8-bab2-b7306e38e4a5
#= function greedy_seam(energies, starting_pixel::Int)
	m,n = size(energies) # row, col 
...
# ╔═╡ 761e2844-5274-4209-929f-e20beeb2710e
grant_example
...
# ╔═╡ f9a695de-af55-4e28-943c-8fea6f285eb5
typeof(size(grant_example,1))
...
function least_energy(energies, i, j) # (i,j) => (row, col)
	...
	if i == m # boundary      
		...
		return (energies[i,j], j) # (lowest energy, column to jump to)        
    ...
    tmpV = [(round((energies[i, j] + least_energy(energies, i+1, j+col)[1]), digits=2), j+col) for col in -1:1 if(checkbounds(Bool, energies, i+1, j+col)) ]
    minTuple = minimum(tmpV)
    minEnergy = minTuple[1]
    minCol = minTuple[2] # position of min in vector

    #println("pos:[", i,",",j,"], ", "value:", energies[i, j], ",  vec: ",tmpV, "\n\tminEnergy:", minEnergy, ", minCol:", minCol, "\n")
...
# ╔═╡ 3c04fe1a-ca5d-4fb8-a616-9ed2301d83cd
## returns lowest possible sum energy at pixel (i, j), and the column to jump to in row i+1.
...
#=     tmpV = [haskey(memory, (i+1, j+col)) ? (energies[i, j] + memory[(i,j)][1], j+col) : ((energies[i, j] + memoized_least_energy(energies, i+1, j+col, memory)[1]), j+col) 
                for col in -1:1 
                    if (checkbounds(Bool, energies, i+1, j+col)) 
                ] =#
    tmpV=[]
    for col in -1:1 
...
function matrix_memoized_least_energy(energies, i, j, memory::Matrix)
    ...
    if i == m # boundary         
        ...
        return memory[i,j]# (lowest energy, column to jump to) *** Took out (i,j)
    end
