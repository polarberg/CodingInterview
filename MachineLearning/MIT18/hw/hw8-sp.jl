...
# ╔═╡ 2d73012c-75b6-409f-aaa9-697764f013a3
...
# ╔═╡ fa53171f-982c-4aaa-b9cd-179478b0b83b
md"""
Answer: If you make it too small then it becomes a line with slope zero
"""
...
# ╔═╡ d3c94a25-05a1-45d8-9f05-a5cacb1de1c8
theoretical = [i^3 for i in 0:0.1:10]
...
# ╔═╡ ed850b80-7e7e-422f-940d-dad3a6b3b2c8
abs(euler_test[50] - 5^3) # > 20

# ╔═╡ 146b65b3-e5b4-4fb2-a279-29cf119d0461
abs(euler_test[end] - 10^3) # > 100

# ╔═╡ ab72fdbe-10be-11eb-3b33-eb4ab41730d6
@bind N_euler Slider(1:40)
...
I don't see a rapid growth. Number of susceptible people decreases, but the number infected stays stable. 
The number recovered has a sigmoud shape where it changes 2nd derivative at around 30 
"""
...
# ╔═╡ 91908e54-7264-473d-a5b4-9f321b222cc7
r = 0:0.01:500
...
# ╔═╡ 0832297c-db70-464e-a617-0ebde64bc184
md"""
γ = $(@bind γ Slider(0:.01:1, show_value = true, default=0.1))
"""

# ╔═╡ e35d57fd-5f93-422a-88c1-2f371eef54ed
interactive_SIR = euler_SIR(β,γ,[0.99,0.01,0.00],r)

# ╔═╡ 496d7d6a-3664-4e55-9437-9641d2ce206c
...
# ╔═╡ eebd17db-ea63-42eb-a5d4-a521ab6f4fae
...
#   ans = [gradient_descent_1d_step(f, x0, η = η)]
#    for i in 1:N_steps
#        push!(ans, gradient_descent_1d_step(f, ans[i], η = η))
#    end 
#	return ans[end]
end
...
...When we keep getting the same output from our function ***
"""
...
function gradient_descent_2d_step(f, x0, y0; η=0.01)
	g = η * gradient(f,x0, y0) # 2d vector 
...
md"μ = $(@bind guess_μ Slider(1:0.5:last(dice_x); default = last(dice_x) * 0.4, show_value=true))"

# ╔═╡ 70f0fe9c-124c-11eb-3dc6-e102e68673d9
md"σ = $(@bind guess_σ Slider(0.1:0.5:last(dice_x)/2; default=5.5, show_value=true))"


# ╔═╡ e3d36e54-41a8-4558-9313-22755ea7c24b
#μ = 35.0
#σ = 5.5
...
md"β = $(@bind guess_β Slider(0.00:0.0001:0.1; default = 0.02, show_value=true))"

# ╔═╡ a9630d28-12d2-11eb-196b-773d8498b0bb
md"γ = $(@bind guess_γ Slider(0.00:0.0001:0.01; default = 0.003, show_value=true))"

# ╔═╡ c122c408-1565-4441-a2e8-a126a2598cae
# β = 0.0188
# γ = 0.0022 
...
function loss_sir(β, γ)
	...
	return sum(ans)/3 # sum(ans) # want to minimize the sum of the vector
end

# ╔═╡ ee20199a-12d4-11eb-1c2c-3f571bbb232e
loss_sir(guess_β, guess_γ)

# ╔═╡ 805bb954-d30f-4923-a76f-fcb92b5accbf
let  
	e = [loss_sir(guess_β+i, guess_γ) for i in 0.016:0.0001:0.020]
	e
end 

# ╔═╡ 9a589281-75e7-40da-ba0a-0b1c4b5de90f
gradient_descent_2d_step(loss_sir, 0.01, 0.01, η = (0.01/1000000))

# ╔═╡ 6946cd3f-2a08-4bf5-a666-bca5b0d7fb47
let 
	f= loss_sir
...
found_β, found_γ = let
	...
	ini_β = guess_β
	ini_γ = guess_γ 
	gradient_descent_2d(loss_sir,ini_β,ini_γ,η= ini_β/1000000,digits_to_Round = 5)
end
...
# ╔═╡ cb27f454-774b-43e1-94d6-91b729d83734
Tup = (db = abs(found_β - 0.019), dc = abs(found_γ - 0.0026))

# ╔═╡ dbd30b69-b3d9-49b3-97a5-0f98ddce14c9
Tup[1] > 0.01

# ╔═╡ b5ad524a-4629-4897-a677-be2ceca45ab6
Tup[2] > 0.001
